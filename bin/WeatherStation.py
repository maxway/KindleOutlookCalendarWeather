...
def get_temp():
    ...
    try:                                                                     # 连接超时,6秒，下载文件超时,7秒
        r = requests.get('http://t.weather.itboy.net/api/weather/city/'+config_dic['citycode'],timeout=(6,7))
        r.encoding = 'utf-8'
        tempList = [
...
def draw_horizontal_bar(draw,h_image,now):
    ...
    #显示城市
    draw.text((220, 55), temp_array[0], font = fontSize16, fill = 0)
    #天气图标
    tempTypeIcon = Image.open(rootPath + "/pic/weatherType/" + get_weather_icon(temp_array[4]))
    h_image.paste(tempTypeIcon,(375,30))
    #今日天气
    draw.text((435,20),temp_array[4], font = fontSize25, fill = 0)
    #温度
    todayTemp = replace_low_temp(temp_array[2])+"-"+replace_high_temp(temp_array[3]) +" 度"
...
def draw_todo(draw):
    ...
    """
    # Iterate over the schedule list, using enumerate to get both index and todo item.
    for idx, todo_str in enumerate(scheduleList):
        # Draw the todo item on the image at the specified position.
        # X coordinate is 30, Y coordinate starts at 145 and increases by 30 for each item.
...
def draw_weather(draw,h_image):
    for x in range(0,3):
        draw.text((630,145 + x *155),get_forecast_day(x), font = fontSize20, fill = 0)
        strWeather = temp_array[weather_switch(x)]
        #风力
        windTemp = temp_array[weather_switch(x)+1] + temp_array[weather_switch(x)+2]
        draw.text((690,145 + x *155),windTemp, font = fontSize20, fill = 0)
        #图标
        pathIcon = get_weather_icon(strWeather)
        tempTypeIcon = Image.open(rootPath + "/pic/weatherType/" + pathIcon)
        h_image.paste(tempTypeIcon,(630,180 + x*155))
        #天气
        draw.text((690,188 + x *155),strWeather, font = fontSize25, fill = 0)
        #温度
        forecastTemp = replace_low_temp(temp_array[weather_switch(x)-2])+"-"+replace_high_temp(temp_array[weather_switch(x)-1]) +" 度"
...
def update_time():
    ...
    while True:
        ...
        if int(config_dic['htmlserver']) == 1:
            now_time_path = rootPath.replace("\\","/") +"/nowTime" + str(strtimeHM) +".png"
        ...
        if int(config_dic['htmlserver']) == 0:
            resolution = ",w=" + config_dic['screenresolutionx']
            fbinkStr = "fbink -c -g file=" + now_time_path + resolution +",halign=center,valign=center"
            os.system(fbinkStr)
...
def network_threading():
    ...
    while True:
        ...
        try:
            ...
            print(get_time() + 'Update Vika Sheet..Fail!'+ str(e) + 'Sheet:' + sheet_id, flush=True)
